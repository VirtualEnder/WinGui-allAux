...
#region normal_aux_states 
                aux = new CheckBoxEx[AUX_CHANNELS, 4, iCheckBoxItems];

            startx = 200;
            starty = 60;

            int a, b, c;
            for (c = 0; c < AUX_CHANNELS; c++)
            {
                for (a = 0; a < 3; a++)
                {
            ...
            aux_labels = new System.Windows.Forms.Label[AUX_CHANNELS];
            lmh_labels = new System.Windows.Forms.Label[AUX_CHANNELS, 3];          // aux1-auxMax, L,M,H
            string strlmh = "LMH";
            for (a = 0; a < AUX_CHANNELS; a++)
            {
            ...
            {
             AUX_CHANNELS = 5; // Keep aux options to 32 bit
#region extended_aux_states
                aux = new CheckBoxEx[AUX_CHANNELS, 6, iCheckBoxItems];
            ...
            aux_labels = new System.Windows.Forms.Label[AUX_CHANNELS];
            lmh_labels = new System.Windows.Forms.Label[AUX_CHANNELS, 6];          // aux1-auxMax, 1,2,3,4,5,6
        ...
        private void update_gui()
        {
            ...
            if (tabMain.SelectedIndex == GUIPages.RC)
            {
                ...
                if (!check_capability(CAP.EXTENDED_AUX))
                {
                    ...
                    for (int i = 0; i < AUX_CHANNELS; i++)
                    {
                        auxState |= (mw_gui.rcAUX[i] <= rcLow) ? (UInt32)1 << (3 * i) : 0;
                        auxState |= (rcLow < mw_gui.rcAUX[i] && mw_gui.rcAUX[i] < rcMid) ? (UInt32)1 << (3 * i + 1) : 0;
                        auxState |= (mw_gui.rcAUX[i] > rcMid) ? (UInt32)1 << (3 * i + 2) : 0;
                    }
                    UInt32 check_state;
                    for (int b = 0; b < iCheckBoxItems; b++)
                    {
                        ...
                        {
                            for (int i = 0; i < AUX_CHANNELS; i++) {
                                if (aux[i, a, b].Checked) check_state += (UInt32)(1 << (3 * i + a));
                            }
                        }
                ...
                else
                {
                    ...
                    for (int b = 0; b < iCheckBoxItems; b++)
                    {
                        ...
                        {
                            for (int i = 0; i < AUX_CHANNELS; i++) {
                                if (aux[i, a, b].Checked) check_state += (UInt32)(1 << (6 * i + a));
                            }
                        }
                        //Highlight active function name
                        if ( (check_state & auxState) != 0) { cb_labels[b].BackColor = Color.Red; cb_labels[b].ForeColor = Color.Yellow; } else { cb_labels[b].BackColor = Color.Transparent; cb_labels[b].ForeColor = Color.White; }
        ...
        private void update_params()
        {
            ...
            for (int b = 0; b < iCheckBoxItems; b++)
            {
                ...
                if (!check_capability(CAP.EXTENDED_AUX))
                {
                    for (byte a = 0; a < 3; a++)
                    {
                        for (byte c = 0; c < AUX_CHANNELS; c++)
                        {
                            if (aux[c, a, b].Checked) mw_params.activation[b] += (UInt32)(1 << (3 * c + a));
                        }
                ...
                else
                {
                    for (byte a = 0; a < 6; a++)
                    {
                        for (byte c = 0; c < AUX_CHANNELS; c++)
                        {
                            if (aux[c, a, b].Checked) mw_params.activation[b] += (UInt32)(1 << (6 * c + a));
                        }
                    }
                }
        ...
        private void update_aux_panel()
        {
            ...
            if (!check_capability(CAP.EXTENDED_AUX))
            {
                ...
                {
                    int k = 0; int l = 0; 
                    for (int j = 0; j < (AUX_CHANNELS * 3); j++)
                    {
                        aux[k, l, i].Checked = (mw_gui.activation[i] & (1 << j)) == 0 ? false : true;
                        l++;
                        if (l == 3) { k++; l = 0; }
                    }
                }
                ...
                for (int i = 0; i < iCheckBoxItems; i++)
                {
                    ...
                    for (int j = 0; j < (AUX_CHANNELS * 3); j++)
                    {
                        aux[k, l, i].IsHighlighted = (aux[k, l, i].Checked == ((mw_gui.activation[i] & (1 << j)) == 0)) ? true : false;
                        l++;
                        if (l == 3) { k++; l = 0; }
                    }
                }
            ...
            else
            {
                ...
                {
                    int k = 0; int l = 0; 
                    for (int j = 0; j < (AUX_CHANNELS * 6); j++)
                    {
                        aux[k, l, i].Checked = (mw_gui.activation[i] & (1 << j)) == 0 ? false : true;
                        l++;
                        if (l == 6) { k++; l = 0; }
        ...
        private void update_gui_from_params()
        {
            if (!check_capability(CAP.EXTENDED_AUX))
            {
                for (int i = 0; i < iCheckBoxItems; i++)
                {
                    ...
                    for (int j = 0; j < (AUX_CHANNELS * 3); j++)
                    {
                        aux[k, l, i].Checked = (mw_params.activation[i] & (1 << j)) == 0 ? false : true;
                        l++;
                        if (l == 3) { k++; l = 0; }
                    }
                }
